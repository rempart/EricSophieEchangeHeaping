##--------------------------------------------------------------------------------------------------------
## SCRIPT : Answer reviewers with adhoc simulations 
##
## Authors : Sophie Donnet & Eric Parent
##
...
for(l in rev(distrib)) {
  ...
  for(j in rev(n_obs)) {
    ...
    for(i in 1:n_sim) {
      set.seed(seeds[i])
      all_covar[[i]] <- make_Xcov(n_obs = j*coeff_mult, n_observer = n_observer, n_cov = n_cov)
      ...
      all_data[[i]] <- do.call('rbind', temp) %>% 
        arrange(X) %>%
        filter(X<seuil) %>%
        sample_n(j)
        rm(temp)
    }
    save(list = c("all_data"), file = paste("res_Eric/simul_data_", l, "_n", j, ".RData", sep = ""))
  }
}
...
model = 'Poisson'
load(paste0('res_Eric/simul_data_',model,'_n', max(n_obs),'.RData'))
data_heap <- do.call("rbind", lapply(1:n_sim,function(l){all_data[[l]]$sim=l; return(all_data[[l]])}))
data_heap <- data_heap %>%mutate(Observer =as.factor(Observer)) %>% mutate(Small = as.factor(ifelse(Y<100,'Y < 100','Y >= 100')))
...
data_heap %>% filter(X>=seuillage) %>% ggplot(aes(x=X,y=Y,color=as.factor(sim_G))) + geom_point() + scale_x_continuous(trans='log') + scale_y_continuous(trans='log') + geom_abline(intercept =0, slope=1)
data_heap %>% filter(X <seuillage) %>% ggplot(aes(x=X,y=Y,color=as.factor(sim_G))) + geom_point()  + geom_abline(intercept =0, slope=1)
...
data_heap %>% mutate(RMSE_X_Y= sqrt((X-Y)^2/X^2)*100) %>% filter(sim_G == 4) %>%filter(X<seuillage)%>% ggplot(aes(x=X,y=RMSE_X_Y,color=as.factor(sim_G))) + geom_point() + facet_grid(.~as.factor(sim_G),scales="free")
data_heap%>% filter(X>=seuillage) %>% ggplot(aes(x=X)) + geom_histogram() 
data_heap%>% filter(X>=seuillage)  %>% ggplot(aes(x=Y)) + geom_histogram() 
...
get_bayes <- function(all_data,n,distrib,n_chain = 4,n_iter=2000,n_warm=1000,n_thin=2,hmc_control=list(adapt_delta = 0.9)) 
  ...
  for(i in 1:length(all_data)) {
    print(paste("Traitement du jeu",i))
    ### null model
  ...
  save(file = paste("res_Eric/stan_data_", distrib, "_n", n, ".RData", sep = ""),
...
for(l in distrib) {
  ...
  for(j in c((n_obs))) {
    message(paste("\t sample size:", j, sep = " "))
    load(paste("res_Eric/simul_data_", l, "_n", j, ".RData", sep = ""))
    get_bayes(all_data = lapply(1:n_sim,function(index){all_data[[index]]}), n = j, distrib = l)
  }
}; rm(i, j, l); Sys.time()-tdeb
...
# load("~/Documents/GitHub/DauphinsPapier/EricSophieEchangeHeaping/res_Eric/stan_data_Poisson_n100CINQUANTEetUNaCENT.RData")
# poisson_nullA=poisson_null
# poisson_heapA=poisson_heap
# negbin_nullA=negbin_null
# load("~/Documents/GitHub/DauphinsPapier/EricSophieEchangeHeaping/res_Eric/stan_data_Poisson_n100VINGTSIXaCINQUANTE.RData")
# poisson_nullA=c(poisson_null,poisson_nullA)
# poisson_heapA=c(poisson_heap,poisson_heapA)
# negbin_nullA=c(negbin_null,negbin_nullA)
# load("~/Documents/GitHub/DauphinsPapier/EricSophieEchangeHeaping/res_Eric/stan_data_Poisson_n100UNaVINGTCINQ.RData")
# poisson_nullA=c(poisson_null,poisson_nullA)
# poisson_heapA=c(poisson_heap,poisson_heapA)
# negbin_nullA=c(negbin_null,negbin_nullA)
# poisson_null<-poisson_nullA
# poisson_heap<-poisson_heapA
# negbin_null<-negbin_nullA
# length(poisson_null)
# length(poisson_heap)
# length(negbin_null)
# rm(list = c("poisson_nullA", "negbin_nullA", "poisson_heapA"))
# save(file = paste("res_Eric/stan_data_Poisson_n100",".RData", sep = ""),
#      list = c("poisson_null", "negbin_null", "poisson_heap"))
# rm(list = c("poisson_null", "negbin_null", "poisson_heap"))

# load("~/Documents/GitHub/DauphinsPapier/EricSophieEchangeHeaping/res_Eric/stan_data_Poisson_n50UNaCINQUANTE.RData")
# poisson_nullA=poisson_null
# poisson_heapA=poisson_heap
# negbin_nullA=negbin_null
# load("~/Documents/GitHub/DauphinsPapier/EricSophieEchangeHeaping/res_Eric/stan_data_Poisson_n50.RData")
# poisson_null=c(poisson_nullA,poisson_null)
# poisson_heap=c(poisson_heapA,poisson_heap)
# negbin_null=c(negbin_nullA,negbin_null)
# length(poisson_null)
# length(poisson_heap)
# length(negbin_null)
# rm(list = c("poisson_nullA", "negbin_nullA", "poisson_heapA"))
# save(file = paste("res_Eric/stan_data_Poisson_n50",".RData", sep = ""),
#      list = c("poisson_null", "negbin_null", "poisson_heap"))
# rm(list = c("poisson_null", "negbin_null", "poisson_heap"))
