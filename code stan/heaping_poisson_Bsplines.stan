...
data {
  ...
  matrix[n_obs, n_cov] Z;                // design matrix that excludes the intercept
  int<lower = 1> n_knot;
  matrix[n_obs, n_knot + 1] B[n_cov];    // Beziers-splines basis, formulated as mixed model (Eilers & Marx 2010)
  real<lower = 0.0> prior_scale_smooth;
}
...
parameters {
  ...
  vector<lower = 0.0>[n_cov] tau_smooth;
  vector<lower = 0.0>[n_cov] unscaled_sigma2_smooth;
  vector[n_knot + 1] unscaled_smooth[n_cov];
}
...
transformed parameters {
  ...
  vector<lower = 0.0>[n_cov] sigma;
  vector[n_knot + 1] smooth[n_cov];
  ...
  for(k in 1:n_cov) {
    sigma[k] = prior_scale_smooth * sqrt(unscaled_sigma2_smooth[k] / tau_smooth[k]);
    smooth[k] = sigma[k] * unscaled_smooth[k];
    linpred += B[k] * smooth[k]; 
  }
...
model {
  ...
  tau_smooth ~ gamma(1.0, 1.0);
  unscaled_sigma2_smooth ~ gamma(0.5, 1.0);
  for(k in 1:n_cov) {
    unscaled_smooth[k] ~ normal(0.0, 1.0);
  }
  for(i in 1:n_obs) {
    target += log_lik[i];
  }
